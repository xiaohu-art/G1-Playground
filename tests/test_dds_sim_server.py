...
class TestG1MujocoDdsServer(unittest.TestCase):
    ...
    def test_active_command_uses_full_pd_clip_and_support_schedule(self):
        backend = FakeBackend()
        bridge = FakeBridge(command(valid=True, age_seconds=0.0))
        server = make_server(backend, bridge)

        for now in (10.0, 13.0, 15.5, 18.0):
            server.step(now=now)
        ...
        self.assertEqual(backend.read_calls, 1)
        np.testing.assert_allclose(backend.scales, [1.0, 1.0, 0.5, 0.0], rtol=0.0, atol=1e-12)
